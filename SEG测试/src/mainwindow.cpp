    ...
    , ui(new Ui::MainWindow)
{
    ui->setupUi(this);
    ui->percent->hide();
    ui->Tip->appendPlainText("Infinity合成引擎测试工具 v1.0.0\n作者：無常\n");
}
...
void MainWindow::on_openfile_clicked()
{
    ...
    ui->filepath->setText(QFileDialog::getOpenFileName(this,"选择引擎文件",currentpath,"Infinity合成引擎(*.dll);;所有文件(*.*)"));
}
...
void MainWindow::on_loadengine_clicked()
{
    ...
    if(!ui->filepath->text().isEmpty()){
        if(library->isLoaded()&&segptr!=nullptr){
            if(running){
                ...
                disconnect(pb,SIGNAL(segfinished()),this,SLOT(on_finished()));
                Stop(segptr);
                while (Check(segptr)) {
    ...
    if(library->isLoaded()){
        if(segptr!=nullptr){
            if(running){
                ...
                connect(pb,SIGNAL(segfinished()),this,SLOT(on_finished()));
                Run(segptr,ui->track->value(),ui->time->value(),ui->tempo->value(),ui->clamp->value(),ui->fs->currentText().toInt(),ui->ain->text());
                pb->start();
                ui->percent->setValue(0);
                ui->percent->show();
                running=true;
                ui->Tip->appendPlainText(time.time().toString()+" "+"引擎已启动！\n"+"轨道："+QString::asprintf("%i",ui->track->value())+"\n"+"时长："+QString::asprintf("%.2f",ui->time->value())+"\n"+"曲速："+QString::asprintf("%.2f",ui->tempo->value())+"\n"+"节拍："+QString::asprintf("%i",ui->clamp->value())+"\n"+"采样率："+ui->fs->currentText()+"\n"+"引擎附加信息："+ui->ain->text()+"\n");
            }
        }else{
            ui->Tip->appendPlainText(time.time().toString()+" "+"引擎类指针为空！\n");
        }
