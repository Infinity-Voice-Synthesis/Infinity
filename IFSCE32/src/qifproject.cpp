...
QIFproject::QIFproject()
{
    ...
    tracklist->at(1)->wavesf->at(0)->data->fill(-13,((double)(time+1))*1000);
...
void QIFproject::settime(int time)
{
    ...
    if(QIFproject::time>time){
        ...
        for(int i=1;i<tracklist->size();i++){
            tracklist->at(i)->wavesf->at(0)->data->insert(tracklist->at(i)->wavesf->at(0)->data->size(),((double)(time+1))*1000-((double)(QIFproject::time+1))*1000,-13);
...
void QIFproject::addtrack()
{
    ...
    track->clamp=clamp;
    track->FS=FS;
    track->wavesf->at(0)->data->fill(-13,((double)(time+1))*1000);
